...
from lendsmart.base.exceptions import LendsmartRestException
from lendsmart.compat import urlparse
from lendsmart.http import HttpClient
from lendsmart.http.response import Response
from lendsmart.jwt.validation import ClientValidationJwt
...
class ValidationClient(HttpClient):
    ...
    def __init__(self, account_sid, api_key_sid, credential_sid, private_key, pool_connections=True):
        """
        Build a ValidationClient which signs requests with private_key and allows Lendsmart to
        validate request has not been tampered with.

        :param str account_sid: A Lendsmart Account Sid starting with 'AC'
        :param str api_key_sid: A Lendsmart API Key Sid starting with 'SK'
        :param str credential_sid: A Credential Sid starting with 'CR',
                                   corresponds to public key Lendsmart will use to verify the JWT.
    ...
    def request(self, method, url, params=None, data=None, headers=None, auth=None, timeout=None,
        ...
        :rtype: A :class:`Response <lendsmart.rest.http.response.Response>` object
        """
        ...
        prepared_request.headers['Lendsmart-Client-Validation'] = jwt.to_jwt()
    ...
    def validate_ssl_certificate(self, client):
        ...
        :return: null on success, raise LendsmartRestException if the request fails
        """
        response = client.request('GET', 'https://api.lendsmart.com:8443')

        if response.status_code < 200 or response.status_code >= 300:
            raise LendsmartRestException(response.status_code, 'https://api.lendsmart.com:8443', 'Failed to validate SSL certificate')
